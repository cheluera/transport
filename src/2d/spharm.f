       subroutine spharm(nnmax,nn,mm,cc,theta,sphi,cphi,p,phire,phiim)          
!----------------------------------------------------------------
!	Computes
!
!	Sqrt[(2 n +1)/(4 pi)]*Sqrt[(n-m)!/(n+m)!] P_n^m(cc)
!
!       and its derivative with respect to cc
!
...
!c
!c        if((theta .lt. 1.0d0) .or. ((pi - theta) .lt. 1.0d0)) then
!c	
!c          if(pi - theta .lt. 1.0d0) then
!c	  
!c	    theta = pi - theta
!c	    
!c	  endif
!c
!c          if(int(log10(one/toll)/log10(one/theta)).lt.mm) then
!c	  
!c	    mmax = int(log10(one/toll)/log10(one/theta)) + 1
!c	    
!c	  else
!c	  
!c	    mmax = mm
